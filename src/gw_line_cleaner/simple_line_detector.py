...
class Line:
    ...
    Attributes:
        frequency (float): Peak frequency in Hz
        amplitude (float): Peak amplitude relative to baseline
        left_boundary (float): Left frequency boundary where line returns to baseline
        right_boundary (float): Right frequency boundary where line returns to baseline
        width (float): Frequency width of the line (right_boundary - left_boundary)
        detector (Optional[str]): Optional detector identifier (e.g., "H1", "L1", "V1")
    """
    ...
    def __post_init__(self):
        ...
        if self.width <= 0:
            msg = f"Line width must be positive: {self.width}"
            raise ValueError(msg)
        if self.frequency <= 0:
            msg = f"Frequency must be positive: {self.frequency}"
            raise ValueError(msg)
        if self.amplitude <= 0:
            msg = f"Amplitude must be positive: {self.amplitude}"
            raise ValueError(msg)
...
def detect_lines(
    ...
    Args:
        frequency (np.ndarray): Frequency array in Hz (must be positive and monotonic)
        spectrum (np.ndarray): Measured spectrum values (ASD in 1/√Hz)
        baseline (np.ndarray): Fitted baseline values (optional - will use baseline_fit if not provided)
        min_amplitude (float): Minimum peak height relative to baseline (default 2.5)
        baseline_level (float): Threshold for boundary detection (default 1.05 = 5% above baseline)
        min_prominence (float): Minimum prominence for peak detection (default 0.10)
        detector_name (Optional[str]): Optional detector identifier to attach to detected lines

    Returns:
        List[Line]: List of Line objects sorted by frequency
    ...
    if len(frequency) != len(spectrum) or len(spectrum) != len(baseline):
        msg = "Frequency, spectrum, and baseline arrays must have identical lengths"
        raise ValueError(msg)

    if len(frequency) < 10:
        msg = "Need at least 10 data points for reliable line detection"
        raise ValueError(msg)
    ...
    if not np.all(spectrum > 0) or not np.all(baseline > 0):
        msg = "All spectrum and baseline values must be positive"
        raise ValueError(msg)

    # Calculate normalized spectrum (ratio to baseline)
    # This removes the overall spectral shape and highlights deviations
    ratio = np.divide(
        spectrum, baseline, out=np.ones_like(spectrum), where=baseline > 1e-30
    )
    ...
    for peak_idx in peak_indices:
        line = _find_physical_boundaries(
            frequency, ratio, peak_idx, baseline_level, detector_name
        )
        if line is not None:
...
def detect_lines_multiple(
    detector_data: Dict[str, Tuple[np.ndarray, np.ndarray, np.ndarray]], **kwargs
) -> Dict[str, List[Line]]:
    """Detect lines for multiple detectors.

    Args:
        detector_data (Dict[str, Tuple[np.ndarray, np.ndarray, np.ndarray]]):
            Dictionary mapping detector names to (frequency, spectrum, baseline) tuples
        **kwargs: Additional arguments passed to detect_lines

    Returns:
        Dict[str, List[Line]]: Dictionary mapping detector names to lists of detected Lines
    """
    results = {}
    for det_name, (freq, spectrum, baseline) in detector_data.items():
        results[det_name] = detect_lines(
            freq, spectrum, baseline, detector_name=det_name, **kwargs
        )
    return results
...
def _find_physical_boundaries(
    ...
    Args:
        ...
        ratio (np.ndarray): Spectrum-to-baseline ratio array
        peak_idx (int): Index of peak center in the arrays
        baseline_level (float): Threshold ratio for determining line boundaries
        detector_name (Optional[str]): Optional detector identifier

    Returns:
        Optional[Line]: Line object if valid boundaries found, None otherwise
    """
...
def _get_frequency_threshold(frequency: float) -> float:
    ...
    Returns:
        float: Frequency ratio threshold (e.g., 1.02 = 2% frequency difference)
    """
...
def _get_gap_threshold(frequency: float) -> float:
    ...
    Returns:
        float: Maximum relative gap as fraction of frequency (e.g., 0.005 = 0.5%)
    """
...
def _should_merge_lines(line1: Line, line2: Line) -> bool:
    ...
    Args:
        line1 (Line): Line objects to compare
        line2 (Line): Line objects to compare

    Returns:
        bool: True if lines should be merged, False otherwise
    """
...
def _logarithmic_clustering(lines: List[Line]) -> List[Line]:
    ...
    Args:
        lines (List[Line]): List of candidate Line objects

    Returns:
        List[Line]: List of unique Line objects after clustering
    """
...
def _merge_two_lines(line1: Line, line2: Line) -> Line:
    ...
    Args:
        line1 (Line): Line objects to merge
        line2 (Line): Line objects to merge

    Returns:
        Line: New Line object representing the merged result
    ...
    if merged_width <= 0:
        msg = f"Merged line has invalid width: {merged_width}"
        raise ValueError(msg)
...
def plot_result(
    ...
    Args:
        ...
        lines (List[Line]): List of detected Line objects
        title (Optional[str]): Plot title (auto-generated if None)
        save_path (Optional[str]): Optional file path to save plot
        detector_name (Optional[str]): Optional detector identifier for labeling
    """
    ...
    plt.loglog(frequency, baseline, "r-", linewidth=2.5, label="Baseline Fit")
    ...
    if lines:
        ...
        for line in lines:
            plt.axvspan(
                line.left_boundary, line.right_boundary, alpha=0.15, color="orange"
            )
...
def find_and_plot(
    ...
    Args:
        frequency (np.ndarray): Frequency array (Hz)
        spectrum (np.ndarray): Spectrum data (ASD)
        baseline (np.ndarray): Fitted baseline (optional - will use baseline_fit if not provided)
        min_amplitude (float): Minimum peak amplitude threshold
        baseline_level (float): Baseline crossing threshold
        min_prominence (float): Minimum peak prominence
        save_path (Optional[str]): Optional path to save plot
        detector_name (Optional[str]): Optional detector identifier for labeling

    Returns:
        List[Line]: List of detected Line objects
    """
...
def detect_from_fit(
    frequency: np.ndarray,
    spectrum: np.ndarray,
    fit_result: baseline_fit.Result,
    min_amplitude: float = GLOBAL_MIN_AMPLITUDE,
    baseline_level: float = GLOBAL_BASELINE_LEVEL,
    min_prominence: float = GLOBAL_MIN_PROMINENCE,
    save_path: Optional[str] = None,
    ...
    Args:
        frequency (np.ndarray): Frequency array (Hz)
        spectrum (np.ndarray): Original spectrum data
        fit_result (baseline_fit.Result): Result object from baseline_fit.fit()
        min_amplitude (float): Minimum peak amplitude threshold
        baseline_level (float): Baseline crossing threshold
        min_prominence (float): Minimum peak prominence
        save_path (Optional[str]): Optional path to save plot
