...
def clean_lines_from_PSD(
    ...
    Args:
        freq (np.ndarray): Frequency array (Hz), common to all detectors
        detector_psds (Dict[str, np.ndarray]):
            Dict mapping detector name to PSD array
                e.g., {"H1": h1_psd, "L1": l1_psd, "V1": v1_psd}
        detector_clean (Dict[str, np.ndarray]):
            Dict mapping detector name to cleaning array (modified in place)
                Values of 0 mean no cleaning; non-zero values are replacement PSD values
        out_dir (Optional[Path]): Output directory for CSV files (optional, set to None to skip file output)
        out_name (str): Base name for output files
        min_detectors (int): Minimum detectors for a line to be considered coherent (not cleaned)
        export_csv (bool): Whether to export diagnostic CSV files
    ...
    for det_name in detector_names:
        if len(detector_psds[det_name]) != n_points:
            msg = (
                f"PSD array for {det_name} has wrong length: "
    ...
    try:
        ...
        detector_baselines_psd = {
            det: baseline**2 for det, baseline in detector_baselines_asd.items()
        }
        ...
        for det_name in detector_names:
            ...
            for line in det_lines:
                if line.frequency not in det_coherent_freqs:
                    ...
                    line_mask = (freq >= line.left_boundary) & (
                        freq <= line.right_boundary
                    )
                    idxs = np.where(line_mask)[0]
                    if len(idxs) > 0:
                        map_idx = np.searchsorted(
                            det_freq_grid, freq[idxs], side="left"
                        )
            ...
            for det_name in detector_names:
                _write_csv(
                    ...
                    (
                        (
                            float(freq[i]),
                            int(detector_clean[det_name][i] != 0.0),
                            (
                                float(detector_clean[det_name][i])
                                if detector_clean[det_name][i] != 0.0
                                else 0.0
                            ),
                        )
...
def _export_analysis_results(
    ...
    detector_freqs: Dict,
    detector_spectra: Dict,
    detector_baselines_asd: Dict,
    detector_lines: Dict,
    coherent_freqs: Dict,
    ...
    for det_name in detector_names:
        ...
        _write_csv(
            ...
            (
                (
                    float(det_freq[i]),
                    float(det_spectrum[i]),
                    float(det_baseline_asd[i]),
                    float(
                        det_spectrum[i] / det_baseline_asd[i]
                        if det_baseline_asd[i] > 0
                        else 1.0
                    ),
                )
        ...
        _write_csv(
            ...
            ["freq_Hz", "ASD"],
            (
                (float(det_freq[i]), float(det_spectrum[i]))
                for i in range(len(det_freq))
            ),
        )
    ...
    _write_csv(
        ...
        (
            (
                ...
                f"{max(line.frequency for line in group.detector_lines.values()) - min(line.frequency for line in group.detector_lines.values()):.8f}",
            )
    ...
    for det_name in detector_names:
        ...
        summary_rows.append(
            (det_name, total, narrow, medium, wide, coherent, total - coherent)
        )
...
def get_cleaning_mask(
    ...
    Args:
        ...
        detector_psds (Dict[str, np.ndarray]): Dict mapping detector names to PSD arrays
        min_detectors (int): Minimum detectors for coherence

    Returns:
        Dict[str, np.ndarray]: Dict mapping detector names to boolean cleaning masks
